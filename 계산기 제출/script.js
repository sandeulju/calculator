...
let firstNum, operatorForAdvanced, previousKey, previousNum, result;

// fristNum : 첫번째 보이는 숫자
// operatorForAdvanced : 연산자
// previousKey = 전에 누른 버튼
// previousNum = 두번째 숫자를 클릭할 때 숫자
// result = 계산된 결과 값;
...
buttons.addEventListener('click', function (event) {
  ...
  if (target.matches('button')) {
    if (action === 'number') {
     // 숫자를 누르는 경우 동작
        // display.textContent != '0'이면 => firstNum = firstNum + buttonContent;
     
        // previousNum = firstNum
      // 숫자가 0인 경우 0을 버튼콘텐츠로 변경
    
      // 연산자가 없는 경우 첫번째 숫자
      if(operatorForAdvanced === undefined){
        // display.textContent = '0'이면 => firstNum = buttonContent; 
        ...
        // 이전의 입력값이 빈칸이 아니고 연산자가 아닌경우
        if( previousKey === '0' || previousKey === '1' || previousKey === '2' || previousKey === '3' || previousKey === '4' || previousKey === '5' || previousKey === '6' || previousKey === '7' || previousKey === '8' || previousKey === '9' || previousKey === '.'){
          previousKey = buttonContent;
    ...
    if (action === 'operator') {
      // 첫번째 숫자가 있고 두번째 숫자가 없으면 연산자 입력 
      if(firstNum !== undefined && previousNum === undefined){
    ...
    if (action === 'decimal') {}
    if (action === 'clear') {
      // 클리어 버튼 클릭 시 display.textContent = '0';
      // firstNum = undefined
